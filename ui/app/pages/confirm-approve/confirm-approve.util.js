...
export function getCustomTxParamsData (data, { customPermissionAmount, decimals }) {
  ...
  if (!tokenData) {
    ...
    throw new Error(`Invalid data; should have be 'approve' method, but instead is '${tokenData.name}'`)
  }
  ...
  let customPermissionValue = decimalToHex(calcTokenValue(Number(customPermissionAmount), decimals))
